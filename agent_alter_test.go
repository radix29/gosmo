...
func TestScriptAgentAlterWrites(t *testing.T) {
	...
	runScriptCases(t, []scriptCase{
		{"Job Alter, three fields", func(c context.Context) error {
			return job().AlterContext(c, JobChanges{
		...
		{"Job Alter sends a zero value it was given", func(c context.Context) error {
			return job().AlterContext(c, JobChanges{Description: Ptr(""), StartStepID: Ptr(0)})
		...
		{"Alert Alter, trigger and delay", func(c context.Context) error {
			return alert().AlterContext(c, AlertChanges{
				ErrorNumber:           Ptr(823),
				Severity:              Ptr(0),
		...
		{"Alert Alter clears the category as [Uncategorized]", func(c context.Context) error {
			return alert().AlterContext(c, AlertChanges{Category: Ptr("")})
		...
		{"Schedule Alter, frequency and range in one call", func(c context.Context) error {
			return schedule().AlterContext(c, ScheduleChanges{
				Enabled: Ptr(true),
		...
		{"Schedule Alter from a Ref addresses by name", func(c context.Context) error {
			return (&Server{}).ScheduleRef("Daily'2am").AlterContext(c, ScheduleChanges{Enabled: Ptr(false)})
...
func TestAgentAlterEmptyChangesIssuesNothing(t *testing.T) {
	cases := []struct {
		...
		{"Job", func(c context.Context) error {
			return (&Job{server: &Server{}, Name: "J"}).AlterContext(c, JobChanges{})
		}},
		{"Alert", func(c context.Context) error {
			return (&Alert{server: &Server{}, Name: "A"}).AlterContext(c, AlertChanges{})
		}},
		{"Operator", func(c context.Context) error {
			return (&Operator{server: &Server{}, Name: "O"}).AlterContext(c, OperatorChanges{})
		}},
		{"Schedule", func(c context.Context) error {
			return (&Schedule{server: &Server{}, ID: 7, Name: "S"}).AlterContext(c, ScheduleChanges{})
		}},
	}
	for _, c := range cases {
...
func TestAgentAlterMirrorsOnlyWhatItSet(t *testing.T) {
	...
	if err := j.AlterContext(ctx, JobChanges{Name: Ptr("New"), Description: Ptr("new")}); err != nil {
		t.Fatalf("AlterContext under WithScript: %v", err)
	}
	...
	if err := a.AlterContext(context.Background(), AlertChanges{NotificationMessage: Ptr("msg")}); err != nil {
