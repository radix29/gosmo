...
var goldenQueries = []goldenQuery{
	...
	{"index_columns", []string{"index_columns"}, nil, func(m int) string {
		return (&Table{db: dbAtMajor(m)}).indexColumnsSelect()
	}},
	{"columns", []string{"columns"}, nil, func(m int) string {
		return dbAtMajor(m).columnSelect()
	}},
	{"parameters", []string{"parameters"}, nil, func(m int) string {
		return dbAtMajor(m).parameterSelect()
	}},
