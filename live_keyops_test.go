...
func TestLiveKeyOps(t *testing.T) {
	...
	if err := c1.Backup(ctx, CertificateBackupSpec{File: file("_c1pub.cer")}); err != nil {
		t.Fatal(err)
	}
	if err := c1.Backup(ctx, CertificateBackupSpec{File: file("_c1.cer"), PrivateKeyFile: file("_c1.pvk"), EncryptionPassword: "Bk!111aaaBBB"}); err != nil {
		t.Fatal(err)
	}
	if err := c2.Backup(ctx, CertificateBackupSpec{File: file("_c2.cer"), PrivateKeyFile: file("_c2.pvk"),
		EncryptionPassword: "Bk!111aaaBBB", DecryptionPassword: cp}); err != nil {
		t.Fatal(err)
	}
	if got, _ := d.CertificateByName(ctx, "c2"); got == nil || got.PvtKeyLastBackupDate.IsZero() {
	...
	if mods, err := d.SignableModules(ctx); err != nil || len(mods) != 2 || mods[0].Name != "f1" || mods[1].Name != "p1" {
		t.Errorf("SignableModules = %v, %v; want f1 and p1, the inline function left out", mods, err)
	}
	if on, err := d.SignaturesOn(ctx, "dbo", "p1"); err != nil || len(on) != 2 {
		t.Errorf("SignaturesOn(p1) = %d, %v; want 2", len(on), err)
	}
	if by, err := c1.SignedModules(ctx); err != nil || len(by) != 1 || by[0].Module != "p1" {
	...
	// -- Private key removal.
	if err := c2.RemovePrivateKey(ctx); err != nil {
		t.Fatal(err)
	}
	if err := d.AsymmetricKeyRef("a1").RemovePrivateKey(ctx); err != nil {
		t.Fatal(err)
	}
	if got, _ := d.CertificateByName(ctx, "c2"); got == nil || got.HasPrivateKey() {
		t.Error("c2 still has a private key")
	}
	if got, _ := d.AsymmetricKeyByName(ctx, "a1"); got == nil || got.HasPrivateKey() {
	...
	if err := m.DropEncryption(ctx, MasterKeyEncryptor{ServiceMasterKey: true}, ""); err != nil {
		t.Fatal(err)
	}
	// Not encrypted by the service master key any more, so every write needs
	// it opened by password.
	if err := m.AddEncryption(ctx, MasterKeyEncryptor{ServiceMasterKey: true}, ""); liveMsg(err) != 15581 {
		t.Errorf("adding the SMK encryption unopened: %v, want Msg 15581", err)
	}
	if err := m.AddEncryption(ctx, MasterKeyEncryptor{ServiceMasterKey: true}, "wrong"); liveMsg(err) != 15313 {
		t.Errorf("adding the SMK encryption with a wrong password: %v, want Msg 15313", err)
	}
	if err := m.Backup(ctx, file("_dmk_open.key"), "Bk!111aaaBBB", mk); err != nil {
		t.Fatal(err)
	}
	if err := m.AddEncryption(ctx, MasterKeyEncryptor{ServiceMasterKey: true}, mk); err != nil {
		t.Fatal(err)
	}
	if m, _ := d.MasterKey(ctx); m == nil || !m.EncryptedByServer {
		t.Error("the SMK encryption did not come back")
	}
	if err := m.AddEncryption(ctx, MasterKeyEncryptor{Password: "Mk!222cccDDD"}, ""); err != nil {
		t.Fatal(err)
	}
	if err := m.DropEncryption(ctx, MasterKeyEncryptor{Password: mk}, ""); err != nil {
		t.Fatal(err)
	}
	if err := m.Backup(ctx, file("_dmk.key"), "Bk!111aaaBBB", ""); err != nil {
		t.Fatal(err)
	}
	if err := m.Regenerate(ctx, "Mk!333eeeFFF", false, ""); err != nil {
		t.Fatal(err)
	}
	if err := m.DropEncryption(ctx, MasterKeyEncryptor{Password: "Mk!333eeeFFF"}, ""); liveMsg(err) != 15558 {
		t.Errorf("dropping the last password: %v, want Msg 15558", err)
	}
	if err := m.Drop(ctx); liveMsg(err) != 15580 {
	...
	if m, err := d.MasterKey(ctx); m != nil || err != nil {
