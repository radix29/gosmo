...
classDiagram
    ...
    class ConnectionOptions {
        ...
        +AccessToken string
        +AccessTokenProvider func(ctx) (string, error)
        +ServerSPN string
    ...
    class Server {
        ...
        +ParseServerAddress(server) (host, instance, port)
        +Close() error
        +DB() *sql.DB
        +Info() *ServerInfo
        +Name() string
    ...
    class KerberosOptions {
        ...
        Windows: native SSPI unless set.
        Every other platform: AuthWindows
        authenticates via Kerberos, using
        this (or the ambient kinit cache
        when the zero value).
    }
    ...
    class withRetry {
        ...
        (attempt * 50ms). Used by Database.query
        and Database.queryRow.
    }
    ...
    class Quoting {
        ...
        one quoting implementation. quoteIdent
        (internal) delegates to QuoteName.
    }
    ...
    class ScriptCollector {
        +Statements []string
        +WithScript(ctx) (ctx, *ScriptCollector)
